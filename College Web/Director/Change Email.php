...
if (empty($_SESSION['Username'] && $_SESSION['Password']) || $_SESSION['Login_type'] != 'Director') {        
...
if(isset($_POST['Reset']))
{
    ...
    //Director
    $sql = "SELECT Email FROM `Login` WHERE Login_type='Director'";
   ...
   if($_SESSION['Login_type'] == "Director")
   {
    while($Match = $check->fetch_assoc())
    {
        ...
        echo "<center><strong style='color: white; background-color: red;'>Enter Different Email. Your Old And New both Emails are same.</strong><br></center>";
        $Count = 1;
}
        }
    }
    ...
    //Director new Email
    if($_SESSION['Login_type'] == 'Director' && $Count != 1){
        $sql = "UPDATE `Login` SET Email='$Email' WHERE Login_Type='Director'";
...
<center>
                <p>
            <input type="Email" name="Email" id="Email" placeholder="Enter New Email *" style="height: 19.9px; width: 96%;" required><br></center><br>
