...
if(isset($_POST['Reset']))
{
    ...
    //Principal
    $sql = "SELECT Email FROM `Student Registration` WHERE Enrollment_No=$Username";
   ...
   if($_SESSION['Login_type'] == 'Student')
   {
    ...
    //Faculty new Email
    if($_SESSION['Login_type'] == 'Student' && $Count != 1){
        $sql = "UPDATE `Student Registration` SET Email='$Email' WHERE Enrollment_No='$Username'";
        $Result=mysqli_query($conn, $sql);
        if($Result){
        echo "<center><h4 style='color: white; background-color: green;'>";
        echo " Email has been changed.</h4></center>";
       ...
       //Mail Sent
        ...
        //redirection
         echo "<script>window.location='Welcome_User.php'</script>";
    }
}
//Close Connection
$conn->close();
...
<center>
                <p>
            <input type="Email" name="Email" id="Email" placeholder="Enter New Email *"style="height: 19.9px; width: 96%;" required> <b r></center><br>
